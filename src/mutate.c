    ...
    flyr_t *ctx, callback_t callback)
{
    ...
    if (start_offset == 0 && errno != 0) {
        err("Failed to parse JSON mutation start offset as a number");
        return FAILURE;
    }
    ...
    if (stop_offset == 0 && errno != 0) {
        err("Failed to parse JSON mutation stop offset as a number");
        return FAILURE;
    }

    if ((size_t)stop_offset > ctx->buffer.size) {
        err("Bitflip stop offset exceeds end of buffer");
        return FAILURE;
    }
...
static int handle_mutation(struct json_value_t *action_json_value, flyr_t *ctx, callback_t callback)
{
    const char *action = json_object_get_string(json_object(action_json_value), "action");
    if (!action) {
        err("Failed to parse mutation action from JSON");
        return FAILURE;
    }
    ...
    err("Erroneous mutation action: %s", action);
    return FAILURE;
}
...
int iterate_mutations(flyr_t *ctx, callback_t callback)
{
    ...
    for (ctx->mutations->idx = 0; ctx->mutations->idx < ctx->mutations->count; ctx->mutations->idx++) {
        ...
        if (!action_json_value) {
            err("Failed to parse mutation json (idx: %lu", ctx->mutations->idx);
            return FAILURE;
        }
